...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Disaster level of this person is 'Heroes Association'."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis user is member of 'God'."
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe Disaster level of this person is 'Dragons'."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe Disaster level of this person is 'Demons'."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThe Disaster level of this person is 'Tigers'."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThe Disaster level of this person is 'Wolves'."
        disaster_level_present = True
    elif user.id == 1829047705:
         text += "\n\nOwner Of A Bot. Queen Of @xyzseth. Bot Name Inspired From 'JoJo'."
         disaster_level_present = True
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/OnePunchUpdates/192"),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/OnePunchUpdates/155")
                        ],
                    ]
                ),
                parse_mode=ParseMode.HTML,
...
def stats(update: Update, context: CallbackContext):
    stats = "❂ <b>Stats For <a href='https://t.me/SethRobot'>Seth Robot</a>:</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
